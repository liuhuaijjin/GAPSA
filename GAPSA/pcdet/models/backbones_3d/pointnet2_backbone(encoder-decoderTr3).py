...
class PointNet2MSG(nn.Module):
        ...
        for k in range(self.model_cfg.SA_CONFIG.NPOINTS.__len__()):
            ...
            self.SA_modules.append(
                pointnet2_modules.PointnetSAModuleMSG(
                    ...
                    grid_size=self.model_cfg.SA_CONFIG.GRID_SIZE[k],
                    input_channels=self.model_cfg.SA_CONFIG.INPUT_CHANNELS[k]
                )
            )
            self.SA_modules2.append(
                pointnet2_modules.PointnetSAModuleMSG2(
                    npoint=self.model_cfg.SA_CONFIG.NPOINTS[k],
                    radii=self.model_cfg.SA_CONFIG.RADIUS[k],
                    nsamples=self.model_cfg.SA_CONFIG.NSAMPLE[k],
                    mlps=mlps,
    ...
    def forward(self, batch_dict):
        ...
        l_xyz2, l_features2 = [xyz], [features]
        ...
        for i in range(len(self.SA_modules2)):
            li_xyz, li_features = self.SA_modules2[i](l_xyz2[i], l_features2[i])
            l_xyz2.append(li_xyz)
            l_features2.append(li_features)
        ...
        point_features += point_features2
