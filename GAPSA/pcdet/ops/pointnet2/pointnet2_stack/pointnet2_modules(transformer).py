...
class PyramidModuleV1(nn.Module):
    ...
    def __init__(self, input_channels, nsamples, radius, grid_sizes, num_heads, head_dims, attention_op, dp_value = 0.1, tr_mode = 'Tr'):
        """
        Args:
            nsamples: list of int, number of samples in each ball query
            mlps: list of list of int, spec of the pointnet before the global pooling for each scale
        ...
        assert self.tr_mode in ['NoTr', 'Normal', 'Residual', 'Tr']
        ...
        for i in range(self.num_pyramid_levels):
            ...
            self.pe.append(nn.Sequential(
                nn.Conv1d(self.pos_dims, self.output_dims, 1)
                #nn.Conv1d(self.pos_dims, self.output_dims, 1, groups=1, bias=False),
                #nn.ReLU()
            ))
            ...
            if self.tr_mode != 'NoTr':
                ...
                self.chunk.append(
                    nn.TransformerEncoder(
                        TransformerEncoderLayerPreNorm(d_model=self.output_dims, dim_feedforward=2 * self.output_dims, dropout=0.1, nhead=4), num_layers=2
                    )
                )
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        for i in range(self.num_pyramid_levels):
            ...
            #     new_features = new_features.permute(1, 0, 2).contiguous()  # (B, L, C)
                ...
                new_features = new_features.squeeze(1).reshape(batch_size,num_rois,-1).permute(1,0,2).contiguous()  # (N, B, C)
                new_features = self.transformer_encoders[i](new_features)
                new_features = new_features.permute(1, 0, 2).contiguous()  # (B, N, C)
            ...
            elif self.tr_mode == 'Tr':
                new_xyz_flipped = new_xyz.reshape(batch_size,num_rois,3).transpose(1,2)
                input_features = new_features.squeeze(1).reshape(batch_size,num_rois,new_features.shape[-1]).transpose(0,1)
                input_features += self.pe[i](new_xyz_flipped).permute(2,0,1) #(N,B,C)
                new_features = self.chunk[i](input_features)
                new_features = new_features.permute(1,0,2).contiguous() #(B,N,C)
            # elif self.tr_mode == 'Tr':
            #     new_xyz_flipped = new_xyz.reshape(batch_size,num_rois,3).transpose(1,2)
            #     new_features = new_features.permute(1, 0, 2).contiguous()  # (L, N, C)
            #     new_features += self.pe[i](new_xyz_flipped).permute(0,2,1).reshape(batch_size*num_rois,-1).unsqueeze(0)
            #     new_features = self.chunk[i](new_features)
            #     new_features = new_features.permute(1,0,2).contiguous() #(B,L,C)
            else:
                raise NotImplementedError
...
class TransformerEncoderLayerPreNorm(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        src = self.norm1(src)
        src2, mask = self.self_attn(src, src, src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask)
        src = src + self.dropout1(src2)
        src = self.norm2(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + self.dropout2(src2)
