...
class PyramidModuleV1(nn.Module):
    ...
    def __init__(self, input_channels, nsamples, radius, grid_sizes, num_heads, head_dims, attention_op, dp_value = 0.1, tr_mode = 'NoTr'):
        """
        Args:
            nsamples: list of int, number of samples in each ball query
            mlps: list of list of int, spec of the pointnet before the global pooling for each scale
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        for i in range(self.num_pyramid_levels):
            ...
            #attend_features = value_embedding.sum(-1)
...
class PyramidModuleV3(nn.Module):

    def __init__(self, input_channels, num_heads, head_dims, attention_op, dp_value = 0.1, tr_mode = 'NoTr'):
        """
        Args:
            nsamples: list of int, number of samples in each ball query
            mlps: list of list of int, spec of the pointnet before the global pooling for each scale
        ...
        for i in range(self.num_pyramid_levels):
            ...
            self.pos_proj.append(nn.Sequential(
                #nn.Conv1d(self.pos_dims, self.output_dims, 1, groups=1, bias=False),
                nn.Linear(self.pos_dims, self.output_dims, bias=False),
                nn.ReLU(),
            ))
            self.key_proj.append(nn.Sequential(
                #nn.Conv1d(input_channels, self.output_dims, 1, groups=1, bias=False),
                nn.Linear(input_channels, self.output_dims, bias=False),
                nn.ReLU()
            ))
            self.value_proj.append(nn.Sequential(
                #nn.Conv1d(input_channels, self.output_dims, 1, groups=1, bias=False),
            ...
            self.attention_proj.append(nn.Sequential(
                #nn.Conv1d(self.output_dims, self.num_heads, 1, groups=1, bias=False),
                nn.Linear(self.output_dims, self.num_heads, bias=False),
            ))
            self.norm_layer.append(nn.Softmax(dim=-1))
            self.k_coef.append(nn.Sequential(
                nn.Linear(self.output_dims, self.output_dims, bias=False),
    ...
    def forward(self, new_xyz, new_features):
        """
        :param xyz: (N1 + N2 ..., 3) tensor of the xyz coordinates of the features
        :param xyz_batch_cnt: (batch_size), [N1, N2, ...]
        :param new_xyz_list: [(B, N x grid_size^3, 3)]
        ...
        for i in range(self.num_pyramid_levels):
            batch_size = new_xyz.shape[0]
            num_rois = new_xyz.shape[1]
            #new_xyz = new_xyz.permute(0,2,1).contiguous() #(B,3,npoints)
            new_xyz = new_xyz.view(-1, 3).contiguous()  # (B*npoints,3)
            new_features = new_features.permute(0,2,1).view(batch_size*num_rois,-1).contiguous() #(B*npoints,64)

            pos_embedding = self.pos_proj[i](new_xyz) #(B,64,npoints)
            key_embedding = self.key_proj[i](new_features)
            value_embedding = self.value_proj[i](new_features)
            pos_key_embedding = pos_embedding * key_embedding

            v_coef = self.v_coef[i](pos_embedding) #(B*npoints,64)
            q_coef = self.q_coef[i](pos_embedding)
            k_coef = self.k_coef[i](key_embedding)
            qk_coef = self.qk_coef[i](pos_key_embedding)

            value_embedding = value_embedding + pos_embedding * v_coef.unsqueeze(2) #(B,64,npoints)
            #value_embedding = value_embedding + pos_embedding
            attention_embedding = pos_embedding * q_coef.unsqueeze(2) + key_embedding * k_coef.unsqueeze(2)# + pos_key_embedding * qk_coef.unsqueeze(2)
            #attention_embedding = pos_embedding + key_embedding + pos_key_embedding
            attention_map = self.attention_proj[i](attention_embedding) #(B,num_heads,npoints)
            #attention_map = self.attention_proj[i](pos_embedding)
            attention_map = self.norm_layer[i](attention_map)
            #(N, num_heads, ns) -> (N, num_heads, head_dims, ns) -> (N, num_heads * head_dims, ns)
            #attention_map = attention_map.unsqueeze(2).repeat(1, 1, self.head_dims, 1).reshape(attention_map.shape[0], -1, attention_map.shape[-1])
            attention_map = attention_map.unsqueeze(2).repeat(1,1,self.head_dims).reshape(batch_size*num_rois,-1)

            attend_features = attention_map * value_embedding
            #attend_features = value_embedding.sum(-1)
