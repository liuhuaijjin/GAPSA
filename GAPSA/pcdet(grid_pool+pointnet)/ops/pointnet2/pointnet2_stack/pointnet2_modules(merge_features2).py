...
class PyramidModuleV1(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        for i in range(self.num_pyramid_levels):
            ...
            grouped_features_src = grouped_features
            BN,C,S=grouped_features_src.shape
            grouped_features_src = grouped_features_src.reshape(batch_size,-1,C,S).permute(0,2,1,3)
            ...
            reg_features_list2.append(grouped_features_src)

        reg_features = torch.cat(reg_features_list, dim=1)  # (B x N, \sum(grid_size^3), C)
        reg_features2 = torch.cat(reg_features_list2, dim=1)

        return reg_features.contiguous(), reg_features2.contiguous()
