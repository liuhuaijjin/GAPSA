...
class QueryAndGroupPyramidAttentionV1(nn.Module):
    def __init__(self, nsample):
        super().__init__()
        self.nsample = nsample
        self.mlp = nn.Sequential(
            nn.Conv1d(10,3,1,groups=1,bias=False),
            nn.BatchNorm1d(3),
            nn.ReLU()
        )
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_r, new_xyz_batch_cnt, features):
        ...
        with torch.no_grad():
            idx, empty_ball_mask = ball_query_deform(self.nsample, xyz, xyz_batch_cnt, new_xyz, new_xyz_r, new_xyz_batch_cnt)
        grouped_xyz_src = grouping_operation(xyz, xyz_batch_cnt, idx, new_xyz_batch_cnt)  # (M1 + M2, 3, nsample)
        grouped_xyz = grouped_xyz_src - new_xyz.unsqueeze(-1)
        grouped_dist = torch.sqrt((grouped_xyz**2).sum(dim=1,keepdim=False))
        extended_new_xyz = new_xyz.unsqueeze(-1).expand(-1,-1,self.nsample)

        grouped_xyz = torch.cat((extended_new_xyz, grouped_xyz_src, grouped_xyz, grouped_dist),dim=1)
        grouped_xyz[empty_ball_mask] = 0

        assert features is not None
        grouped_features = grouping_operation(features, xyz_batch_cnt, idx, new_xyz_batch_cnt)  # (M1 + M2, C, nsample)
        grouped_features[empty_ball_mask] = 0
        grouped_features = torch.cat([self.mlp(grouped_xyz), grouped_features], dim=1)  # (M1 + M2 ..., C + 3, nsample)
