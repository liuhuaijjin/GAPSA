...
class PyramidModuleV1(nn.Module):
    ...
    def __init__(self, input_channels, nsamples, radius, grid_sizes, num_heads, head_dims, attention_op, dp_value = 0.1, tr_mode = 'NoTr'):
        ...
        for i in range(self.num_pyramid_levels):
            ...
            self.mlp.append(nn.Sequential(
                nn.Conv1d(input_channels+self.output_dims, self.output_dims, 1, groups=1, bias=False),
                nn.BatchNorm1d(self.output_dims),
                nn.ReLU(),
            ))
    ...
    def forward(self, xyz, new_xyz, features=None, new_point_features=None):
        """
        :param xyz: (N1 + N2 ..., 3) tensor of the xyz coordinates of the features
        :param xyz_batch_cnt: (batch_size), [N1, N2, ...]
        :param new_xyz_list: [(B, N x grid_size^3, 3)]
        ...
        for i in range(self.num_pyramid_levels):
            ...
            reg_features = torch.cat((reg_features,new_point_features.transpose(1,2),new_xyz.transpose(1,2)),dim=1)
            reg_features = self.mlp[i](reg_features)
